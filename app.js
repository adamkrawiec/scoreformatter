...
class VolleyballParser {
    ...
    get score() {
        let [mainScore, ...setScores] = str.split(',');
        
        let setScoresFormatted = setScores.map((setScore, i) => `set${i + 1} ${setScore}`);
        return `Main score: ${mainScore} (setScoresFormatted)`;
    }
}

class HandballParser {
